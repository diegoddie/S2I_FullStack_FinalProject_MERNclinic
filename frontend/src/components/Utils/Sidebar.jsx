...
const Sidebar = ({ data, isAdmin, selectedSection, handleMenuItemClick }) => {
    ...
    return (
        <div className={`bg-secondary rounded-md py-3 px-3 mx-auto w-full duration-200 relative ${open ? 'md:w-[310px]' : 'md:w-[120px]'}`}>
            <MdMenuOpen className={`bg-white hidden md:flex absolute cursor-pointer duration-300 rounded-full -right-3 top-14 w-9 h-9 border-4 border-secondary text-3xl ${!open && 'rotate-180'}`} onClick={()=>setOpen(!open)}/>
